...
public sealed class CompletePurchaseHandler : IRequestHandler<CompletePurchaseCommand, Result>
{
    private readonly IPurchaseOrderReadStore _readStore;
    private readonly IPurchaseOrderWriteStore _writeStore;
    private readonly IEventBus _eventBus;

    public CompletePurchaseHandler(IPurchaseOrderReadStore readStore, IPurchaseOrderWriteStore writeStore, IEventBus eventBus)
    {
    ...
    public async Task<Result> Handle(CompletePurchaseCommand request, CancellationToken cancellationToken)
    {
        ...
        await _eventBus.PublishAsync(new PurchaseCompletedIntegrationEvent(order.Id, order.PurchaseNumber, order.TotalAmount, order.CompletedAt!.Value), cancellationToken);
